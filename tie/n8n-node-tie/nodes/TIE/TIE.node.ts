...
export class TIE implements INodeType {
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const returnData: INodeExecutionData[] = new Array(items.length);

		// Process items in chunks to prevent unbounded concurrency
		const concurrencyLimit = 50;
		const executeInChunks = async () => {
			for (let i = 0; i < items.length; i += concurrencyLimit) {
				const chunkEnd = Math.min(i + concurrencyLimit, items.length);
				const chunkPromises: Array<Promise<void>> = [];

				for (let itemIndex = i; itemIndex < chunkEnd; itemIndex += 1) {
					const processItem = async () => {
						...
						if (operation === 'health') {
							...
							returnData[itemIndex] = { json: health, pairedItem: { item: itemIndex } };
							return;
						}
						...
						const output: IDataObject = {
							request: requestBody as unknown as IDataObject,
							response,
							decision: response.decision ?? null,
							status: response.status ?? response.decision ?? 'unknown',
						};
						returnData[itemIndex] = { json: output, pairedItem: { item: itemIndex } };
					};

					chunkPromises.push(processItem());
				}
				...
				for (let j = 0; j < results.length; j++) {
					...
					if (result.status === 'rejected') {
						const index = i + j;
						returnData[index] = {
							json: { error: result.reason.message || result.reason },
